...
3.  **Tiêu chuẩn Cauchy**
    ...
    **Ví dụ**: Cho dãy $x_{n}\colon x_{1}=\sqrt{2},$
    $x_{n+1}=\sqrt{2+x_{n}}$. Chứng minh rằng $\left\{ x_{n}\right\}$
...
Cho dãy $\left\{ x_{n}\right\}$, dãy con
$\left\{ x_{k_{n}}\right\} \subset\left\{ x_{n}\right\}$,
$x_{k_{n}}\rightarrow l$ , khi $n\rightarrow\infty$, thì $l$ được gọi là
giới hạn riêng của dãy $\left\{ x_{n}\right\} .$

-   Giới hạn riêng lớn nhất (nếu có) của dãy số thực
    $\left\{ x_{n}\right\}$ được gọi là **giới hạn trên** của dãy
...
2.  Định lý 2: Dãy số thực $\left\{ x_{n}\right\}$ có giới hạn (hữu hạn
