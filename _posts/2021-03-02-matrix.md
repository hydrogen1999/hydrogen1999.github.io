...
- Quy chuẩn:
    $$
        x_i^*={x_i\over\sqrt{\sigma_{ii}}} \qquad \Rightarrow d(O,P)=\sqrt{{x_1^*}^2+\dots+{x_d^*}^2}
    $$
...
#### 19. Bổ đề cực đại hóa

Cho $B$ là ma trận xác định dương cấp $d$, khi đó $\forall a\in\mathbb{R}^d$

$$
    \max_{x\ne 0}{{(x^Ta)^2}\over{x^TBx}}=a^TB^{-1}a, \quad \text{dấu bằng đạt tại }x=cB^{-1}a.
$$

#### 20. Cực đại toàn phương trên hình cầu đơn vị

Cho $B$ là ma trận xác định dương cấp $d$ có các cặp ($\lambda_i,e_i$), $i=\overline{1,d}$ với $\lambda_1\ge \lambda_2\ge \dots\ge \lambda_d\ge 0$; khi đó:

$$
    \max_{x\ne 0}{{x^TBx}\over{x^Tx}}=\lambda_1 \quad \text{tại }x=e_1
$$

$$
    \min_{x\ne 0}{{x^TBx}\over{x^Tx}}=\lambda_d \quad \text{tại }x=e_d
$$

Ngoài ra,

$$
    \max_{x\perp e_1,\dots,e_k}{{x^TBx}\over{x^Tx}}=\lambda_{k+1} \quad \text{tại }x=e_{k+1} , k=\overline{1,d-1}.
$$
